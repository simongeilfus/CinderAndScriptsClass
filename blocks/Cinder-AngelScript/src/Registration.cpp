...
namespace as {
	...
	void registerCinder( asIScriptEngine* engine, bool useNamespaces )
	{
        ...
        int r = engine->RegisterObjectBehaviour( "Vec2f", asBEHAVE_IMPLICIT_VALUE_CAST, "Vec2i f() const", asFUNCTION((BindingHelper::cast<Vec2f,Vec2i>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
        r = engine->RegisterObjectBehaviour( "Vec2i", asBEHAVE_IMPLICIT_VALUE_CAST, "Vec2f f() const", asFUNCTION((BindingHelper::cast<Vec2i,Vec2f>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	}
