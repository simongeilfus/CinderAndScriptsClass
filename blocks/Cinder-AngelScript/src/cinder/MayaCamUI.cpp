...
namespace as {
	...
	void registerCinderMayaCamUIImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "MayaCamUI", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<MayaCamUI>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "MayaCamUI", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<MayaCamUI>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "MayaCamUI", asBEHAVE_CONSTRUCT,
			"void f(const MayaCamUI &in )", asFUNCTION( ( as::BindingHelper::constructor<MayaCamUI,const MayaCamUI &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "MayaCamUI","MayaCamUI& opAssign(const MayaCamUI &in)", asFUNCTION((as::BindingHelper::opAssign<MayaCamUI&,MayaCamUI,MayaCamUI&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: MayaCamUI( const CameraPersp &aInitialCam )
		r = engine->RegisterObjectBehaviour( "MayaCamUI", asBEHAVE_CONSTRUCT,
			"void f( const CameraPersp &in aInitialCam )", asFUNCTION( ( as::BindingHelper::constructor<MayaCamUI,const CameraPersp &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
