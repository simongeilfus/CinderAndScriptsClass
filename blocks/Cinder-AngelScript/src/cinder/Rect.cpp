...
namespace as {
	...
	void registerCinderRectTImpl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<RectT<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<RectT<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<RectT<T>,const RectT<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<RectT<T>&,RectT<T>,RectT<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

/*
		// CONSTRUCTOR: RectT( const std::vector<Vec2<T> > &points )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const std::vector<Vec2" + suffix + " > &in points )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<RectT<T>,const std::vector<Vec2<T> > &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
*/
		// CONSTRUCTOR: RectT( T aX1, T aY1, T aX2, T aY2 )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " aX1, " + type + " aY1, " + type + " aX2, " + type + " aY2 )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<RectT<T>,T ,T ,T ,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// CONSTRUCTOR: RectT( const Vec2<T> &v1, const Vec2<T> &v2 )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Vec2" + suffix + " &in v1, const Vec2" + suffix + " &in v2 )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<RectT<T>,const Vec2<T> &,const Vec2<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// CONSTRUCTOR: RectT( const Area &area )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Area &in area )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<RectT<T>,const Area &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
...
*/
		...
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( const Vec2" + suffix + " &in o )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const RectT<T>,RectT<T>,const Vec2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const RectT<T> operator-( const Vec2<T> &o ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( const Vec2" + suffix + " &in o )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const RectT<T>,RectT<T>,const Vec2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const RectT<T> operator*( T s ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const RectT<T>,RectT<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const RectT<T> operator/( T s ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const RectT<T>,RectT<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: const RectT<T> operator+( const RectT<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const RectT<T>,RectT<T>,const RectT<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const RectT<T> operator-( const RectT<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const RectT<T>,RectT<T>,const RectT<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: RectT<T>& operator+=( const Vec2<T> &o )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( const Vec2" + suffix + " &in o )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<RectT<T>&,RectT<T>,const Vec2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: RectT<T>& operator-=( const Vec2<T> &o )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( const Vec2" + suffix + " &in o )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<RectT<T>&,RectT<T>,const Vec2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: RectT<T>& operator*=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<RectT<T>&,RectT<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: RectT<T>& operator/=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<RectT<T>&,RectT<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderRectMappingImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "RectMapping", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<RectMapping>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "RectMapping", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<RectMapping>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "RectMapping", asBEHAVE_CONSTRUCT,
			"void f(const RectMapping &in )", asFUNCTION( ( as::BindingHelper::constructor<RectMapping,const RectMapping &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "RectMapping","RectMapping& opAssign(const RectMapping &in)", asFUNCTION((as::BindingHelper::opAssign<RectMapping&,RectMapping,RectMapping&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// PROPERTY: public:
		//  : mSrcRect( 0, 0, 0, 0 ), mDstRect( 0, 0, 0, 0 )
		// CONSTRUCTOR: RectMapping( const Rectf &aSrcRect, const Rectf &aDstRect )
		r = engine->RegisterObjectBehaviour( "RectMapping", asBEHAVE_CONSTRUCT,
			"void f( const Rectf &in aSrcRect, const Rectf &in aDstRect )", asFUNCTION( ( as::BindingHelper::constructor<RectMapping,const Rectf &,const Rectf &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( "RectMapping", asBEHAVE_CONSTRUCT,
			"void f( const Rectf &in aSrcRect, const Rectf &in aDstRect, bool preserveSrcAspect )", asFUNCTION( ( as::BindingHelper::constructor<RectMapping,const Rectf &,const Rectf &,bool > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
