...
namespace as {
	...
	void registerCinderPath2dImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Path2d", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Path2d>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Path2d", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Path2d>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Path2d", asBEHAVE_CONSTRUCT,
			"void f(const Path2d &in )", asFUNCTION( ( as::BindingHelper::constructor<Path2d,const Path2d &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Path2d","Path2d& opAssign(const Path2d &in)", asFUNCTION((as::BindingHelper::opAssign<Path2d&,Path2d,Path2d&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
