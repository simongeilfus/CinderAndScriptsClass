...
namespace as {
	...
	void registerCinderShape2dImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Shape2d", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Shape2d>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Shape2d", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Shape2d>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Shape2d", asBEHAVE_CONSTRUCT,
			"void f(const Shape2d &in )", asFUNCTION( ( as::BindingHelper::constructor<Shape2d,const Shape2d &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Shape2d","Shape2d& opAssign(const Shape2d &in)", asFUNCTION((as::BindingHelper::opAssign<Shape2d&,Shape2d,Shape2d&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
