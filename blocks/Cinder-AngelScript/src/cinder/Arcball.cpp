...
namespace as {
	...
	void registerCinderArcballImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Arcball", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Arcball>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Arcball", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Arcball>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Arcball", asBEHAVE_CONSTRUCT,
			"void f(const Arcball &in )", asFUNCTION( ( as::BindingHelper::constructor<Arcball,const Arcball &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Arcball","Arcball& opAssign(const Arcball &in)", asFUNCTION((as::BindingHelper::opAssign<Arcball&,Arcball,Arcball&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );



		// CONSTRUCTOR: Arcball( const Vec2i &aScreenSize )
		r = engine->RegisterObjectBehaviour( "Arcball", asBEHAVE_CONSTRUCT,
			"void f( const Vec2i &in aScreenSize )", asFUNCTION( ( as::BindingHelper::constructor<Arcball,const Vec2i &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
