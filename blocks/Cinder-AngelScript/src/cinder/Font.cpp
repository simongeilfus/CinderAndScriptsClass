...
namespace as {
	...
	void registerCinderFontImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Font", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Font>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Font", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Font>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		/*r = engine->RegisterObjectBehaviour( "Font", asBEHAVE_CONSTRUCT,
			"void f(const Font &in )", asFUNCTION( ( as::BindingHelper::constructor<Font,const Font &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );*/

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Font","Font& opAssign(const Font &in)", asFUNCTION((as::BindingHelper::opAssign<Font&,Font,Font&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( "Font", asBEHAVE_CONSTRUCT,
			"void f( string &in aName, float size )", asFUNCTION( ( as::BindingHelper::constructor<Font,const std::string &,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// PROPERTY: \note Assumes a point size relative to 72dpi on Cocoa but 96dpi on Windows. This creates rough parity between the platforms on type size, but in Windows this renders fonts smaller than normal. **/
		// CONSTRUCTOR: Font( DataSourceRef dataSource, float size )
		/*r = engine->RegisterObjectBehaviour( "Font", asBEHAVE_CONSTRUCT,
			"void f( DataSourceRef dataSource, float size )", asFUNCTION( ( as::BindingHelper::constructor<Font,DataSourceRef ,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
*/
