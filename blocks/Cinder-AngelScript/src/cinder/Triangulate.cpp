...
namespace as {
	...
	void registerCinderTriangulatorImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Triangulator", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Triangulator>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Triangulator", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Triangulator>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Triangulator", asBEHAVE_CONSTRUCT,
			"void f(const Triangulator &in )", asFUNCTION( ( as::BindingHelper::constructor<Triangulator,const Triangulator &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Triangulator","Triangulator& opAssign(const Triangulator &in)", asFUNCTION((as::BindingHelper::opAssign<Triangulator&,Triangulator,Triangulator&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( "Triangulator", asBEHAVE_CONSTRUCT,
			"void f( const Path2d &in path, float approximationScale = 1.0f )", asFUNCTION( ( as::BindingHelper::constructor<Triangulator,const Path2d &,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// CONSTRUCTOR: Triangulator( const Shape2d &shape, float approximationScale = 1.0f )
		r = engine->RegisterObjectBehaviour( "Triangulator", asBEHAVE_CONSTRUCT,
			"void f( const Shape2d &in shape, float approximationScale = 1.0f )", asFUNCTION( ( as::BindingHelper::constructor<Triangulator,const Shape2d &,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// CONSTRUCTOR: Triangulator( const PolyLine2f &polyLine )
		r = engine->RegisterObjectBehaviour( "Triangulator", asBEHAVE_CONSTRUCT,
			"void f( const PolyLine2f &in polyLine )", asFUNCTION( ( as::BindingHelper::constructor<Triangulator,const PolyLine2f &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
