...
namespace as {
	...
	void registerCinderTimerImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Timer", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Timer>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Timer", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Timer>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Timer", asBEHAVE_CONSTRUCT,
			"void f(const Timer &in )", asFUNCTION( ( as::BindingHelper::constructor<Timer,const Timer &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Timer","Timer& opAssign(const Timer &in)", asFUNCTION((as::BindingHelper::opAssign<Timer&,Timer,Timer&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );



		// CONSTRUCTOR: Timer( bool startOnConstruction )
		r = engine->RegisterObjectBehaviour( "Timer", asBEHAVE_CONSTRUCT,
			"void f( bool startOnConstruction )", asFUNCTION( ( as::BindingHelper::constructor<Timer,bool > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
