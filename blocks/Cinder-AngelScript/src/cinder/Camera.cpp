...
namespace as {
	...
	void registerCinderCameraImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		/*r = engine->RegisterObjectBehaviour( "Camera", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Camera>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Camera", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Camera>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Camera", asBEHAVE_CONSTRUCT,
			"void f(const Camera &in )", asFUNCTION( ( as::BindingHelper::constructor<Camera,const Camera &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
*/
		// Copy Assignement
		r = engine->RegisterObjectMethod( "Camera","Camera& opAssign(const Camera &in)", asFUNCTION((as::BindingHelper::opAssign<Camera&,Camera,Camera&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		r = engine->RegisterObjectMethod( "Camera","Camera& opAssign(const CameraPersp &in)", asFUNCTION((as::BindingHelper::opAssign<Camera&,Camera,Camera&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		r = engine->RegisterObjectMethod( "Camera","Camera& opAssign(const CameraOrtho &in)", asFUNCTION((as::BindingHelper::opAssign<Camera&,Camera,Camera&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		r = engine->RegisterObjectMethod( "Camera","Camera& opAssign(const CameraStereo &in)", asFUNCTION((as::BindingHelper::opAssign<Camera&,Camera,Camera&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderCameraPerspImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "CameraPersp", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<CameraPersp>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "CameraPersp", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<CameraPersp>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "CameraPersp", asBEHAVE_CONSTRUCT,
			"void f(const CameraPersp &in )", asFUNCTION( ( as::BindingHelper::constructor<CameraPersp,const CameraPersp &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "CameraPersp","CameraPersp& opAssign(const CameraPersp &in)", asFUNCTION((as::BindingHelper::opAssign<CameraPersp&,CameraPersp,CameraPersp&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: CameraPersp( int pixelWidth, int pixelHeight, float fov ) // constructs screen-aligned camera
		r = engine->RegisterObjectBehaviour( "CameraPersp", asBEHAVE_CONSTRUCT,
			"void f( int pixelWidth, int pixelHeight, float fov )", asFUNCTION( ( as::BindingHelper::constructor<CameraPersp,int ,int ,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( "CameraPersp", asBEHAVE_CONSTRUCT,
			"void f( int pixelWidth, int pixelHeight, float fov, float nearPlane, float farPlane )", asFUNCTION( ( as::BindingHelper::constructor<CameraPersp,int ,int ,float ,float ,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderCameraOrthoImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "CameraOrtho", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<CameraOrtho>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "CameraOrtho", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<CameraOrtho>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "CameraOrtho", asBEHAVE_CONSTRUCT,
			"void f(const CameraOrtho &in )", asFUNCTION( ( as::BindingHelper::constructor<CameraOrtho,const CameraOrtho &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "CameraOrtho","CameraOrtho& opAssign(const CameraOrtho &in)", asFUNCTION((as::BindingHelper::opAssign<CameraOrtho&,CameraOrtho,CameraOrtho&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: CameraOrtho( float left, float right, float bottom, float top, float nearPlane, float farPlane )
		/*r = engine->RegisterObjectBehaviour( "CameraOrtho", asBEHAVE_CONSTRUCT,
			"void f( float left, float right, float bottom, float top, float nearPlane, float farPlane )", asFUNCTION( ( as::BindingHelper::constructor<CameraOrtho,float ,float ,float ,float ,float ,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
...
*/
	...
	void registerCinderCameraStereoImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "CameraStereo", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<CameraStereo>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "CameraStereo", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<CameraStereo>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "CameraStereo", asBEHAVE_CONSTRUCT,
			"void f(const CameraStereo &in )", asFUNCTION( ( as::BindingHelper::constructor<CameraStereo,const CameraStereo &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "CameraStereo","CameraStereo& opAssign(const CameraStereo &in)", asFUNCTION((as::BindingHelper::opAssign<CameraStereo&,CameraStereo,CameraStereo&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( "CameraStereo", asBEHAVE_CONSTRUCT,
			"void f( int pixelWidth, int pixelHeight, float fov )", asFUNCTION( ( as::BindingHelper::constructor<CameraStereo,int ,int ,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: CameraStereo( int pixelWidth, int pixelHeight, float fov, float nearPlane, float farPlane )
		r = engine->RegisterObjectBehaviour( "CameraStereo", asBEHAVE_CONSTRUCT,
			"void f( int pixelWidth, int pixelHeight, float fov, float nearPlane, float farPlane )", asFUNCTION( ( as::BindingHelper::constructor<CameraStereo,int ,int ,float ,float ,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
