...
namespace as {
	...
	void registerCinderVboImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Vbo", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Vbo>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Vbo", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Vbo>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Vbo", asBEHAVE_CONSTRUCT,
			"void f(const Vbo &in )", asFUNCTION( ( as::BindingHelper::constructor<Vbo,const Vbo &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Vbo","Vbo& opAssign(const Vbo &in)", asFUNCTION((as::BindingHelper::opAssign<Vbo&,Vbo,Vbo&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Vbo( GLenum aTarget )
		r = engine->RegisterObjectBehaviour( "Vbo", asBEHAVE_CONSTRUCT,
			"void f( uint aTarget )", asFUNCTION( ( as::BindingHelper::constructor<Vbo,GLenum > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderVboMeshLayoutImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Layout", asBEHAVE_CONSTRUCT,
                                            "void f()", asFUNCTION( as::BindingHelper::constructor<	VboMesh::Layout>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Layout", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<	VboMesh::Layout>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Layout", asBEHAVE_CONSTRUCT,
			"void f(const 	Layout &in )", asFUNCTION( ( as::BindingHelper::constructor<	VboMesh::Layout,const 	VboMesh::Layout &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Layout","	Layout& opAssign(const 	Layout &in)", asFUNCTION((as::BindingHelper::opAssign<	VboMesh::Layout&,	VboMesh::Layout,	VboMesh::Layout&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	/*void registerCinderVboMeshVertexIterImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "VertexIter", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<	VboMesh::VertexIter>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "VertexIter", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<	VboMesh::VertexIter>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "VertexIter", asBEHAVE_CONSTRUCT,
			"void f(const 	VertexIter &in )", asFUNCTION( ( as::BindingHelper::constructor<	VboMesh::VertexIter,const 	VboMesh::VertexIter &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "VertexIter","VertexIter& opAssign(const 	VertexIter &in)", asFUNCTION((as::BindingHelper::opAssign<	VboMesh::VertexIter&,	VboMesh::VertexIter,	VboMesh::VertexIter&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderVboMeshImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "VboMesh", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<VboMesh>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "VboMesh", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<VboMesh>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "VboMesh", asBEHAVE_CONSTRUCT,
			"void f(const VboMesh &in )", asFUNCTION( ( as::BindingHelper::constructor<VboMesh,const VboMesh &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "VboMesh","VboMesh& opAssign(const VboMesh &in)", asFUNCTION((as::BindingHelper::opAssign<VboMesh&,VboMesh,VboMesh&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( "VboMesh", asBEHAVE_CONSTRUCT,
			"void f( uint numVertices, uint numIndices, gl::VboMesh::Layout layout, uint primitiveType )", asFUNCTION( ( as::BindingHelper::constructor<VboMesh,size_t ,size_t ,VboMesh::Layout ,GLenum > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
