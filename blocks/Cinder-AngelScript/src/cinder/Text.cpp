...
namespace as {
	...
	void registerCinderTextLayoutImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "TextLayout", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<TextLayout>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "TextLayout", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<TextLayout>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "TextLayout", asBEHAVE_CONSTRUCT,
			"void f(const TextLayout &in )", asFUNCTION( ( as::BindingHelper::constructor<TextLayout,const TextLayout &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "TextLayout","TextLayout& opAssign(const TextLayout &in)", asFUNCTION((as::BindingHelper::opAssign<TextLayout&,TextLayout,TextLayout&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderTextBoxImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "TextBox", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<TextBox>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "TextBox", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<TextBox>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "TextBox", asBEHAVE_CONSTRUCT,
			"void f(const TextBox &in )", asFUNCTION( ( as::BindingHelper::constructor<TextBox,const TextBox &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "TextBox","TextBox& opAssign(const TextBox &in)", asFUNCTION((as::BindingHelper::opAssign<TextBox&,TextBox,TextBox&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
