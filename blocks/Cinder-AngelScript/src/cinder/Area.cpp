...
namespace as {
	...
	void registerCinderAreaImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Area", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Area>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Area", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Area>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Area", asBEHAVE_CONSTRUCT,
			"void f(const Area &in )", asFUNCTION( ( as::BindingHelper::constructor<Area,const Area &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Area","Area& opAssign(const Area &in)", asFUNCTION((as::BindingHelper::opAssign<Area&,Area,Area&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Area( const Vec2i &UL, const Vec2i &LR )
		r = engine->RegisterObjectBehaviour( "Area", asBEHAVE_CONSTRUCT,
			"void f( const Vec2i &in UL, const Vec2i &in LR )", asFUNCTION( ( as::BindingHelper::constructor<Area,const Vec2i &,const Vec2i &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Area( int32_t aX1, int32_t aY1, int32_t aX2, int32_t aY2 )
		r = engine->RegisterObjectBehaviour( "Area", asBEHAVE_CONSTRUCT,
			"void f( int32 aX1, int32 aY1, int32 aX2, int32 aY2 )", asFUNCTION( ( as::BindingHelper::constructor<Area,int32_t ,int32_t ,int32_t ,int32_t > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectMethod("Area", "const Area opAdd( const Vec2i &in o )", asFUNCTION((as::BindingHelper::opAdd<const Area,Area,const Vec2i &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Area operator-( const Vec2i &o ) const
		r = engine->RegisterObjectMethod("Area", "const Area opSub( const Vec2i &in o )", asFUNCTION((as::BindingHelper::opSub<const Area,Area,const Vec2i &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: const Area operator+( const Area& rhs ) const
		r = engine->RegisterObjectMethod("Area", "const Area opAdd( const Area&in  rhs )", asFUNCTION((as::BindingHelper::opAdd<const Area,Area,const Area& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Area operator-( const Area& rhs ) const
		r = engine->RegisterObjectMethod("Area", "const Area opSub( const Area&in  rhs )", asFUNCTION((as::BindingHelper::opSub<const Area,Area,const Area& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: Area& operator+=( const Vec2i &o )
		r = engine->RegisterObjectMethod("Area", "Area& opAddAssign( const Vec2i &in o )", asFUNCTION((as::BindingHelper::opAddAssign<Area&,Area,const Vec2i &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Area& operator-=( const Vec2i &o )
		r = engine->RegisterObjectMethod("Area", "Area& opSubAssign( const Vec2i &in o )", asFUNCTION((as::BindingHelper::opSubAssign<Area&,Area,const Vec2i &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
