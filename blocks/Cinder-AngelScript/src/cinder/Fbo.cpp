...
namespace as {
	...
	void registerCinderRenderbufferImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Renderbuffer", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Renderbuffer>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Renderbuffer", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Renderbuffer>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Renderbuffer", asBEHAVE_CONSTRUCT,
			"void f(const Renderbuffer &in )", asFUNCTION( ( as::BindingHelper::constructor<Renderbuffer,const Renderbuffer &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Renderbuffer","Renderbuffer& opAssign(const Renderbuffer &in)", asFUNCTION((as::BindingHelper::opAssign<Renderbuffer&,Renderbuffer,Renderbuffer&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( "Renderbuffer", asBEHAVE_CONSTRUCT,
			"void f( int width, int height, uint internalFormat = GL_RGBA8_OES )", asFUNCTION( ( as::BindingHelper::constructor<Renderbuffer,int ,int ,GLenum > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		//  #else
		// CONSTRUCTOR: Renderbuffer( int width, int height, GLenum internalFormat = GL_RGBA8 )
		r = engine->RegisterObjectBehaviour( "Renderbuffer", asBEHAVE_CONSTRUCT,
			"void f( int width, int height, uint internalFormat = GL_RGBA8 )", asFUNCTION( ( as::BindingHelper::constructor<Renderbuffer,int ,int ,GLenum > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		//  #endif

		// CONSTRUCTOR: Renderbuffer( int width, int height, GLenum internalFormat, int msaaSamples, int coverageSamples = 0 )
		r = engine->RegisterObjectBehaviour( "Renderbuffer", asBEHAVE_CONSTRUCT,
			"void f( int width, int height, uint internalFormat, int msaaSamples, int coverageSamples = 0 )", asFUNCTION( ( as::BindingHelper::constructor<Renderbuffer,int ,int ,GLenum ,int ,int > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderFboFormatImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Format", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Fbo::Format>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Format", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Fbo::Format>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Format", asBEHAVE_CONSTRUCT,
			"void f(const Format &in )", asFUNCTION( ( as::BindingHelper::constructor<Fbo::Format,const Fbo::Format &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Format","Format& opAssign(const Format &in)", asFUNCTION((as::BindingHelper::opAssign<Fbo::Format&,Fbo::Format,Fbo::Format&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderFboImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Fbo", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Fbo>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Fbo", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Fbo>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Fbo", asBEHAVE_CONSTRUCT,
			"void f(const Fbo &in )", asFUNCTION( ( as::BindingHelper::constructor<Fbo,const Fbo &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Fbo","Fbo& opAssign(const Fbo &in)", asFUNCTION((as::BindingHelper::opAssign<Fbo&,Fbo,Fbo&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( "Fbo", asBEHAVE_CONSTRUCT,
                                            "void f( int width, int height, gl::Fbo::Format format = gl::Fbo::Format() )", asFUNCTION( ( as::BindingHelper::constructor<Fbo,int ,int ,Fbo::Format > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// CONSTRUCTOR: Fbo( int width, int height, bool alpha, bool color = true, bool depth = true )
		r = engine->RegisterObjectBehaviour( "Fbo", asBEHAVE_CONSTRUCT,
			"void f( int width, int height, bool alpha, bool color = true, bool depth = true )", asFUNCTION( ( as::BindingHelper::constructor<Fbo,int ,int ,bool ,bool ,bool > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
