...
namespace as {
	...
	void registerCinderMatrix33Impl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Matrix33<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Matrix33<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Matrix33<T>,const Matrix33<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<Matrix33<T>&,Matrix33<T>,Matrix33<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " s )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Matrix33<T>,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Vec3" + suffix + " &in vx, const Vec3" + suffix + " &in vy, const Vec3" + suffix + " &in vz )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Matrix33<T>,const Vec3<T> &,const Vec3<T> &,const Vec3<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Matrix22" + suffix + "&in  src )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Matrix33<T>,const Matrix22<T>& > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Matrix33<T>&,Matrix33<T>,const Matrix33<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix33<T>& operator+=( const Matrix33<T> &rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Matrix33<T>&,Matrix33<T>,const Matrix33<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix33<T>& operator-=( const Matrix33<T> &rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Matrix33<T>&,Matrix33<T>,const Matrix33<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: Matrix33<T>& operator*=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Matrix33<T>&,Matrix33<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix33<T>& operator/=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<Matrix33<T>&,Matrix33<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix33<T>& operator+=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Matrix33<T>&,Matrix33<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix33<T>& operator-=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Matrix33<T>&,Matrix33<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: const Matrix33<T> operator*( const Matrix33<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Matrix33<T>,Matrix33<T>,const Matrix33<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix33<T> operator+( const Matrix33<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const Matrix33<T>,Matrix33<T>,const Matrix33<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix33<T> operator-( const Matrix33<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const Matrix33<T>,Matrix33<T>,const Matrix33<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );



		// OPERATOR: const Vec3<T> operator*( const Vec3<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const Vec3" + suffix + " opMul( const Vec3" + suffix + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Vec3<T>,Matrix33<T>,const Vec3<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: const Matrix33<T> operator*( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Matrix33<T>,Matrix33<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix33<T> operator/( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const Matrix33<T>,Matrix33<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix33<T> operator+( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const Matrix33<T>,Matrix33<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix33<T> operator-( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const Matrix33<T>,Matrix33<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
