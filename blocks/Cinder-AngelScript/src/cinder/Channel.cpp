...
namespace as {
	...
	void registerCinderChannelTImpl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<ChannelT<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<ChannelT<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<ChannelT<T>,const ChannelT<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<ChannelT<T>&,ChannelT<T>,ChannelT<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );



		// CONSTRUCTOR: ChannelT( int32_t width, int32_t height )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( int32 width, int32 height )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<ChannelT<T>,int32_t ,int32_t > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
...
/*		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( ImageSourceRef imageSource )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<ChannelT<T>,ImageSourceRef > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
