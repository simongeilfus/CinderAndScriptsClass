...
namespace as {
	...
	void registerCinderDisplayListImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "DisplayList", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<DisplayList>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "DisplayList", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<DisplayList>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "DisplayList", asBEHAVE_CONSTRUCT,
			"void f(const DisplayList &in )", asFUNCTION( ( as::BindingHelper::constructor<DisplayList,const DisplayList &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "DisplayList","DisplayList& opAssign(const DisplayList &in)", asFUNCTION((as::BindingHelper::opAssign<DisplayList&,DisplayList,DisplayList&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
