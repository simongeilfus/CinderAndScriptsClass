...
namespace as {
	...
	void registerCinderClipboardImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Clipboard", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Clipboard>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Clipboard", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Clipboard>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Clipboard", asBEHAVE_CONSTRUCT,
			"void f(const Clipboard &in )", asFUNCTION( ( as::BindingHelper::constructor<Clipboard,const Clipboard &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Clipboard","Clipboard& opAssign(const Clipboard &in)", asFUNCTION((as::BindingHelper::opAssign<Clipboard&,Clipboard,Clipboard&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
