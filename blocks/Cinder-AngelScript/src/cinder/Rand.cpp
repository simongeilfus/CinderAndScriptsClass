...
namespace as {
	...
	void registerCinderRandImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Rand", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Rand>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Rand", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Rand>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Rand", asBEHAVE_CONSTRUCT,
			"void f(const Rand &in )", asFUNCTION( ( as::BindingHelper::constructor<Rand,const Rand &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Rand","Rand& opAssign(const Rand &in)", asFUNCTION((as::BindingHelper::opAssign<Rand&,Rand,Rand&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( "Rand", asBEHAVE_CONSTRUCT,
			"void f( uint seed )", asFUNCTION( ( as::BindingHelper::constructor<Rand,unsigned long > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
