...
namespace as {
	...
	void registerCinderPolyLineImpl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<PolyLine<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<PolyLine<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<PolyLine<T>,const PolyLine<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<PolyLine<T>&,PolyLine<T>,PolyLine<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: PolyLine( const std::vector<T> &aPoints ) : mPoints( aPoints ), mClosed( false )
/*		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( array<Vec2f> &in aPoints )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<PolyLine<T>,const std::vector<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
*/
...
*/
	...
	template<> void registerCinderPolyLineImpl<Vec3f>( asIScriptEngine* engine, const std::string & name, const std::string & type, const std::string &suffix, bool useNamespaces)
	{
		...
		// Default Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
                                            "void f()", asFUNCTION( as::BindingHelper::constructor<PolyLine<Vec3f>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );
        
		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
                                            "void f()", asFUNCTION(as::BindingHelper::destructor<PolyLine<Vec3f>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );
        
		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
                                            ( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<PolyLine<Vec3f>,const PolyLine<Vec3f> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
        
		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<PolyLine<Vec3f>&,PolyLine<Vec3f>,PolyLine<Vec3f>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
        
		// CONSTRUCTOR: PolyLine( const std::vector<Vec3f> &aPoints ) : mPoints( aPoints ), mClosed( false )
        /*		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
         std::string( "void f( array<Vec2f> &in aPoints )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<PolyLine<Vec3f>,const std::vector<Vec3f> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
         */
