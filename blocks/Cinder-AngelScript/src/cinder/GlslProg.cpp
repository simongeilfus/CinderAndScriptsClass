...
namespace as {
	...
	void registerCinderGlslProgImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "GlslProg", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<GlslProg>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "GlslProg", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<GlslProg>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "GlslProg", asBEHAVE_CONSTRUCT,
			"void f(const GlslProg &in )", asFUNCTION( ( as::BindingHelper::constructor<GlslProg,const GlslProg &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "GlslProg","GlslProg& opAssign(const GlslProg &in)", asFUNCTION((as::BindingHelper::opAssign<GlslProg&,GlslProg,GlslProg&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	/*	r = engine->RegisterObjectBehaviour( "GlslProg", asBEHAVE_CONSTRUCT,
			"void f( DataSourceRef vertexShader, DataSourceRef fragmentShader = DataSourceRef()", asFUNCTION( ( as::BindingHelper::constructor<GlslProg,DataSourceRef ,DataSourceRef > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
*/
