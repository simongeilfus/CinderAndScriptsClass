...
namespace as {
	...
	void registerCinderMatrix22Impl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Matrix22<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Matrix22<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Matrix22<T>,const Matrix22<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<Matrix22<T>&,Matrix22<T>,Matrix22<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " s )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Matrix22<T>,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " d0, " + type + " d1, " + type + " d2, " + type + " d3, bool srcIsRowMajor = false )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Matrix22<T>,T ,T ,T ,T ,bool > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );



		// CONSTRUCTOR: Matrix22( const Vec2<T> &vx, const Vec2<T> &vy )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Vec2" + suffix + " &in vx, const Vec2" + suffix + " &in vy )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Matrix22<T>,const Vec2<T> &,const Vec2<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Matrix22<T>&,Matrix22<T>,const Matrix22<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix22<T>& operator+=( const Matrix22<T> &rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Matrix22<T>&,Matrix22<T>,const Matrix22<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix22<T>& operator-=( const Matrix22<T> &rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Matrix22<T>&,Matrix22<T>,const Matrix22<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: Matrix22<T>& operator*=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Matrix22<T>&,Matrix22<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix22<T>& operator/=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<Matrix22<T>&,Matrix22<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix22<T>& operator+=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Matrix22<T>&,Matrix22<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Matrix22<T>& operator-=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Matrix22<T>&,Matrix22<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: const Matrix22<T> operator*( const Matrix22<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Matrix22<T>,Matrix22<T>,const Matrix22<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix22<T> operator+( const Matrix22<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const Matrix22<T>,Matrix22<T>,const Matrix22<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix22<T> operator-( const Matrix22<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const Matrix22<T>,Matrix22<T>,const Matrix22<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );



		// OPERATOR: const Vec2<T> operator*( const Vec2<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const Vec2" + suffix + " opMul( const Vec2" + suffix + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Vec2<T>,Matrix22<T>,const Vec2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: const Matrix22<T> operator*( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Matrix22<T>,Matrix22<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix22<T> operator/( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const Matrix22<T>,Matrix22<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix22<T> operator+( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const Matrix22<T>,Matrix22<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Matrix22<T> operator-( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const Matrix22<T>,Matrix22<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
