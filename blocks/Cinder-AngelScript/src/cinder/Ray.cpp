...
namespace as {
	...
	void registerCinderRayImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Ray", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Ray>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Ray", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Ray>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Ray", asBEHAVE_CONSTRUCT,
			"void f(const Ray &in )", asFUNCTION( ( as::BindingHelper::constructor<Ray,const Ray &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Ray","Ray& opAssign(const Ray &in)", asFUNCTION((as::BindingHelper::opAssign<Ray&,Ray,Ray&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Ray( const Vec3f &aOrigin, const Vec3f &aDirection ) : mOrigin( aOrigin )
		r = engine->RegisterObjectBehaviour( "Ray", asBEHAVE_CONSTRUCT,
			"void f( const Vec3f &in aOrigin, const Vec3f &in aDirection )", asFUNCTION( ( as::BindingHelper::constructor<Ray,const Vec3f &,const Vec3f &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
