...
namespace as {
	...
	void registerCinderSphereImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Sphere", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Sphere>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Sphere", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Sphere>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Sphere", asBEHAVE_CONSTRUCT,
			"void f(const Sphere &in )", asFUNCTION( ( as::BindingHelper::constructor<Sphere,const Sphere &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Sphere","Sphere& opAssign(const Sphere &in)", asFUNCTION((as::BindingHelper::opAssign<Sphere&,Sphere,Sphere&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Sphere( const Vec3f &aCenter, float aRadius ) : mCenter( aCenter ), mRadius( aRadius )
		r = engine->RegisterObjectBehaviour( "Sphere", asBEHAVE_CONSTRUCT,
			"void f( const Vec3f &in aCenter, float aRadius )", asFUNCTION( ( as::BindingHelper::constructor<Sphere,const Vec3f &,float > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
