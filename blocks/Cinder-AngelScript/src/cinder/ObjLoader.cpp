...
namespace as {
	...
	void registerCinderObjLoaderImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "ObjLoader", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<ObjLoader>), asCALL_CDECL_OBJLAST); assert( r >= 0 );
*/
		// Destructor
		r = engine->RegisterObjectBehaviour( "ObjLoader", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<ObjLoader>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "ObjLoader", asBEHAVE_CONSTRUCT,
			"void f(const ObjLoader &in )", asFUNCTION( ( as::BindingHelper::constructor<ObjLoader,const ObjLoader &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "ObjLoader","ObjLoader& opAssign(const ObjLoader &in)", asFUNCTION((as::BindingHelper::opAssign<ObjLoader&,ObjLoader,ObjLoader&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// PROPERTY: **/
		// CONSTRUCTOR: ObjLoader( DataSourceRef dataSource, bool includeUVs = true )
	/*	r = engine->RegisterObjectBehaviour( "ObjLoader", asBEHAVE_CONSTRUCT,
			"void f( DataSourceRef dataSource, bool includeUVs = true )", asFUNCTION( ( as::BindingHelper::constructor<ObjLoader,DataSourceRef ,bool > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: ObjLoader( DataSourceRef dataSource, DataSourceRef materialSource, bool includeUVs = true )
		r = engine->RegisterObjectBehaviour( "ObjLoader", asBEHAVE_CONSTRUCT,
			"void f( DataSourceRef dataSource, DataSourceRef materialSource, bool includeUVs = true )", asFUNCTION( ( as::BindingHelper::constructor<ObjLoader,DataSourceRef ,DataSourceRef ,bool > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
*/
