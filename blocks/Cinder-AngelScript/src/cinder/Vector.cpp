...
namespace as {
	...
	void registerCinderVec2Impl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Vec2<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Vec2<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Vec2<T>,const Vec2<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<Vec2<T>&,Vec2<T>,Vec2<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " nx, " + type + " ny )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Vec2<T>,T ,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const Vec2<T>,Vec2<T>,const Vec2<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec2<T> operator-( const Vec2<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const Vec2<T>,Vec2<T>,const Vec2<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec2<T> operator*( const Vec2<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Vec2<T>,Vec2<T>,const Vec2<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec2<T> operator/( const Vec2<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const Vec2<T>,Vec2<T>,const Vec2<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec2<T>& operator+=( const Vec2<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Vec2<T>&,Vec2<T>,const Vec2<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec2<T>& operator-=( const Vec2<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Vec2<T>&,Vec2<T>,const Vec2<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec2<T>& operator*=( const Vec2<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Vec2<T>&,Vec2<T>,const Vec2<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec2<T>& operator/=( const Vec2<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<Vec2<T>&,Vec2<T>,const Vec2<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec2<T> operator/( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const Vec2<T>,Vec2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec2<T>& operator+=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Vec2<T>&,Vec2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec2<T>& operator-=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Vec2<T>&,Vec2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec2<T>& operator*=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Vec2<T>&,Vec2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec2<T>& operator/=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<Vec2<T>&,Vec2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderVec3Impl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		// Default Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Vec3<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Vec3<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Vec3<T>,const Vec3<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<Vec3<T>&,Vec3<T>,Vec3<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " nx, " + type + " ny, " + type + " nz )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Vec3<T>,T ,T ,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Vec2" + suffix + " &in v2, " + type + " aZ )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Vec3<T>,const Vec2<T> &,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const Vec3<T>,Vec3<T>,const Vec3<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec3<T> operator-( const Vec3<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const Vec3<T>,Vec3<T>,const Vec3<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec3<T> operator*( const Vec3<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Vec3<T>,Vec3<T>,const Vec3<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec3<T> operator/( const Vec3<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const Vec3<T>,Vec3<T>,const Vec3<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec3<T>& operator+=( const Vec3<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Vec3<T>&,Vec3<T>,const Vec3<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec3<T>& operator-=( const Vec3<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Vec3<T>&,Vec3<T>,const Vec3<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec3<T>& operator*=( const Vec3<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Vec3<T>&,Vec3<T>,const Vec3<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec3<T>& operator/=( const Vec3<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<Vec3<T>&,Vec3<T>,const Vec3<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec3<T> operator/( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const Vec3<T>,Vec3<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec3<T>& operator+=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Vec3<T>&,Vec3<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec3<T>& operator-=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Vec3<T>&,Vec3<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec3<T>& operator*=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Vec3<T>&,Vec3<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec3<T>& operator/=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<Vec3<T>&,Vec3<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	void registerCinderVec4Impl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		// Default Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Vec4<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Vec4<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Vec4<T>,const Vec4<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<Vec4<T>&,Vec4<T>,Vec4<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " nx, " + type + " ny, " + type + " nz, " + type + " nw = 0 )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Vec4<T>,T ,T ,T ,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		//  : x( nx ), y( ny ), z( nz ), w( nw )

		// CONSTRUCTOR: Vec4( const Vec3<T>& src, T aW = 0 )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Vec3" + suffix + "&in  src, " + type + " aW = 0 )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Vec4<T>,const Vec3<T>& ,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const Vec4<T>,Vec4<T>,const Vec4<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec4<T> operator-( const Vec4<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const Vec4<T>,Vec4<T>,const Vec4<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec4<T> operator*( const Vec4<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Vec4<T>,Vec4<T>,const Vec4<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec4<T> operator/( const Vec4<T>& rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const Vec4<T>,Vec4<T>,const Vec4<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec4<T>& operator+=( const Vec4<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Vec4<T>&,Vec4<T>,const Vec4<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec4<T>& operator-=( const Vec4<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Vec4<T>&,Vec4<T>,const Vec4<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec4<T>& operator*=( const Vec4<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Vec4<T>&,Vec4<T>,const Vec4<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec4<T>& operator/=( const Vec4<T>& rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( const " + name + "&in  rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<Vec4<T>&,Vec4<T>,const Vec4<T>& >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const Vec4<T> operator/( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const Vec4<T>,Vec4<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec4<T>& operator+=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<Vec4<T>&,Vec4<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec4<T>& operator-=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<Vec4<T>&,Vec4<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec4<T>& operator*=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<Vec4<T>&,Vec4<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: Vec4<T>& operator/=( T rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<Vec4<T>&,Vec4<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
