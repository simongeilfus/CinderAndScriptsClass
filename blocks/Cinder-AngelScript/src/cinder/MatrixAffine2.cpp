...
namespace as {
	...
	void registerCinderMatrixAffine2Impl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<MatrixAffine2<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<MatrixAffine2<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<MatrixAffine2<T>,const MatrixAffine2<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<MatrixAffine2<T>&,MatrixAffine2<T>,MatrixAffine2<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " s )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<MatrixAffine2<T>,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Vec2" + suffix + " &in vx, const Vec2" + suffix + " &in vy, const Vec2" + suffix + " &in vz )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<MatrixAffine2<T>,const Vec2<T> &,const Vec2<T> &,const Vec2<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<MatrixAffine2<T>&,MatrixAffine2<T>,const MatrixAffine2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: MatrixAffine2<T>& operator+=( const MatrixAffine2<T> &rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<MatrixAffine2<T>&,MatrixAffine2<T>,const MatrixAffine2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: MatrixAffine2<T>& operator-=( const MatrixAffine2<T> &rhs )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<MatrixAffine2<T>&,MatrixAffine2<T>,const MatrixAffine2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: MatrixAffine2<T>& operator*=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opMulAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opMulAssign<MatrixAffine2<T>&,MatrixAffine2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: MatrixAffine2<T>& operator/=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opDivAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opDivAssign<MatrixAffine2<T>&,MatrixAffine2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: MatrixAffine2<T>& operator+=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opAddAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opAddAssign<MatrixAffine2<T>&,MatrixAffine2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: MatrixAffine2<T>& operator-=( T s )
		r = engine->RegisterObjectMethod(name.c_str(), std::string( name + "& opSubAssign( " + type + " s )" ).c_str(), asFUNCTION((as::BindingHelper::opSubAssign<MatrixAffine2<T>&,MatrixAffine2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// OPERATOR: const MatrixAffine2<T> operator*( const MatrixAffine2<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const MatrixAffine2<T>,MatrixAffine2<T>,const MatrixAffine2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const MatrixAffine2<T> operator+( const MatrixAffine2<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const MatrixAffine2<T>,MatrixAffine2<T>,const MatrixAffine2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const MatrixAffine2<T> operator-( const MatrixAffine2<T> &rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( const " + name + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const MatrixAffine2<T>,MatrixAffine2<T>,const MatrixAffine2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const Vec2" + suffix + " opMul( const Vec2" + suffix + " &in rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const Vec2<T>,MatrixAffine2<T>,const Vec2<T> &>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opMul( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opMul<const MatrixAffine2<T>,MatrixAffine2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const MatrixAffine2<T> operator/( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opDiv( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opDiv<const MatrixAffine2<T>,MatrixAffine2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const MatrixAffine2<T> operator+( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opAdd( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opAdd<const MatrixAffine2<T>,MatrixAffine2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// OPERATOR: const MatrixAffine2<T> operator-( T rhs ) const
		r = engine->RegisterObjectMethod(name.c_str(), std::string( "const " + name + " opSub( " + type + " rhs )" ).c_str(), asFUNCTION((as::BindingHelper::opSub<const MatrixAffine2<T>,MatrixAffine2<T>,T >)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
