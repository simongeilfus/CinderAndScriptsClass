...
namespace as {
	...
	void registerCinderPerlinImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "Perlin", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Perlin>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "Perlin", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Perlin>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "Perlin", asBEHAVE_CONSTRUCT,
			"void f(const Perlin &in )", asFUNCTION( ( as::BindingHelper::constructor<Perlin,const Perlin &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "Perlin","Perlin& opAssign(const Perlin &in)", asFUNCTION((as::BindingHelper::opAssign<Perlin&,Perlin,Perlin&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Perlin( uint8_t aOctaves = 4 )
		r = engine->RegisterObjectBehaviour( "Perlin", asBEHAVE_CONSTRUCT,
			"void f( uint8 aOctaves = 4 )", asFUNCTION( ( as::BindingHelper::constructor<Perlin,uint8_t > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Perlin( uint8_t aOctaves, int32_t aSeed )
		r = engine->RegisterObjectBehaviour( "Perlin", asBEHAVE_CONSTRUCT,
			"void f( uint8 aOctaves, int32 aSeed )", asFUNCTION( ( as::BindingHelper::constructor<Perlin,uint8_t ,int32_t > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
