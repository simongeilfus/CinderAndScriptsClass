...
namespace as {
	...
	void registerCinderSurfaceTImpl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<SurfaceT<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<SurfaceT<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<SurfaceT<T>,const SurfaceT<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<SurfaceT<T>&,SurfaceT<T>,SurfaceT<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( int32 width, int32 height, bool alpha )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<SurfaceT<T>,int32_t ,int32_t ,bool ,SurfaceChannelOrder > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
/*
		// CONSTRUCTOR: SurfaceT( int32_t width, int32_t height, bool alpha, const SurfaceConstraints &constraints )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( int32 width, int32 height, bool alpha, const SurfaceConstraints &in constraints )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<SurfaceT<T>,int32_t ,int32_t ,bool ,const SurfaceConstraints &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( ImageSourceRef imageSource, const SurfaceConstraints &in constraints = SurfaceConstraintsDefault()" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<SurfaceT<T>,ImageSourceRef ,const SurfaceConstraints &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
...
/*
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void copyFrom( const void f" + suffix + " &in srcSurface, const Area &in srcArea, const Vec2i &in relativeOffset = Vec2i::zero()" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<SurfaceT<T>,const SurfaceT<T> &,const Area &,const Vec2i &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
