...
namespace as {
	...
	void registerCinderAxisAlignedBox3fImpl( asIScriptEngine* engine, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( "AxisAlignedBox3f", asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<AxisAlignedBox3f>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( "AxisAlignedBox3f", asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<AxisAlignedBox3f>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( "AxisAlignedBox3f", asBEHAVE_CONSTRUCT,
			"void f(const AxisAlignedBox3f &in )", asFUNCTION( ( as::BindingHelper::constructor<AxisAlignedBox3f,const AxisAlignedBox3f &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( "AxisAlignedBox3f","AxisAlignedBox3f& opAssign(const AxisAlignedBox3f &in)", asFUNCTION((as::BindingHelper::opAssign<AxisAlignedBox3f&,AxisAlignedBox3f,AxisAlignedBox3f&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: AxisAlignedBox3f( const Vec3f &aMin, const Vec3f &aMax )
		r = engine->RegisterObjectBehaviour( "AxisAlignedBox3f", asBEHAVE_CONSTRUCT,
			"void f( const Vec3f &in aMin, const Vec3f &in aMax )", asFUNCTION( ( as::BindingHelper::constructor<AxisAlignedBox3f,const Vec3f &,const Vec3f &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
