...
namespace as {
	...
	void registerCinderQuaternionImpl( asIScriptEngine* engine, const std::string &name, const std::string &type, const std::string &suffix, bool useNamespaces )
	{
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			"void f()", asFUNCTION( as::BindingHelper::constructor<Quaternion<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Destructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_DESTRUCT,
			"void f()", asFUNCTION(as::BindingHelper::destructor<Quaternion<T>>), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Constructor
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			( "void f(const " + name + " &in )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Quaternion<T>,const Quaternion<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// Copy Assignement
		r = engine->RegisterObjectMethod( name.c_str(), ( name + "& opAssign(const " + name + " &in)" ).c_str(), asFUNCTION((as::BindingHelper::opAssign<Quaternion<T>&,Quaternion<T>,Quaternion<T>&>)), asCALL_CDECL_OBJLAST); assert( r >= 0 );
		...
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " aW, " + type + " x, " + type + " y, " + type + " z )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Quaternion<T>,T ,T ,T ,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// CONSTRUCTOR: Quaternion( const Vec3<T> &axis, T radians )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Vec3" + suffix + " &in axis, " + type + " radians )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Quaternion<T>,const Vec3<T> &,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Quaternion( const Vec3<T> &from, const Vec3<T> &to )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Vec3" + suffix + " &in from, const Vec3" + suffix + " &in to )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Quaternion<T>,const Vec3<T> &,const Vec3<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );


		// CONSTRUCTOR: Quaternion( T xRotation, T yRotation, T zRotation )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( " + type + " xRotation, " + type + " yRotation, " + type + " zRotation )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Quaternion<T>,T ,T ,T > ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Quaternion( const Matrix33<T> &m )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Matrix33" + suffix + " &in m )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Quaternion<T>,const Matrix33<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );

		// CONSTRUCTOR: Quaternion( const Matrix44<T> &m )
		r = engine->RegisterObjectBehaviour( name.c_str(), asBEHAVE_CONSTRUCT,
			std::string( "void f( const Matrix44" + suffix + " &in m )" ).c_str(), asFUNCTION( ( as::BindingHelper::constructor<Quaternion<T>,const Matrix44<T> &> ) ), asCALL_CDECL_OBJLAST); assert( r >= 0 );
